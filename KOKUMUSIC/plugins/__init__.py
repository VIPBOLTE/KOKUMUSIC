...
# Clean up previous extra plugins folder and utils if they exist
...
# If extra plugins are enabled, clone the external repository and handle the utils folder
if extra_plugins_enabled:
    # Clone the external plugins repository if not already cloned
    ...
    # Append the utils path to sys.path if it exists
    if os.path.isdir(utils_target_path):
        sys.path.append(utils_target_path)

    # Install external requirements if necessary
...
# Function to list all plugin modules, including those from the external repository
...
# List all the modules (from the main repo and external plugins)
